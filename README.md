...
Start the server. you need 2 separate terminals run this command:

1. First terminal (Client):
...
```
client/src
├── actions     - Storing functions to call server APIs
├── assets      - contains images that the app need
├── components  - contains re-used React components
├── page        - Handle the client controllers
├── reducers    - A redux state that always listen to changes
├── constant    - Store all constants for the client
├── store       - Initialize Redux store
└── App.js      - contain all routes and initialize the HomePage
```
...
Any user is able to login/register/logout an account
 Goal: A new user can register an account with our app or log in with their Facebook/Google credentials
...
User is able to create any events
 Goal: Registered user clicks a “Create Event” button to create a new event/trip
 Functional Processes:
...
User is able to see a history of changes in the events
 Goal: User can refer back to details (group members, total, location) of past or favorite events
 Functional Processes:
...
User is able to sort their list of events by time
  Goal: User will be able to sort their list of events in chronological order
  Functional Processes:
